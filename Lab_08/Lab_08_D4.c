...
__interrupt void watchdog_timer(void)
{
	...
	char *myPointer = (char* )&myData;			// Send one byte at a time

	UART_putCharacter(0x55);	// Send header
	for(index = 0; index < 4; index++)
	{
		UART_putCharacter(myPointer[index]);	// Send 4-bytes of myData
	}

	myData = (myData + 0.1); 	// Update myData for next transmission
	if(myData >= 10.0)
	{
		myData = 0.0;
	}
